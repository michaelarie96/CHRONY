...
const EventForm = ({ event, onSave, onCancel, onDelete }) => {
  ...
  const calculateActualDuration = () => {
    if (event?.start && event?.end) {
      // Calculate actual duration in minutes from the event's start and end times
      return moment(event.end).diff(moment(event.start), "minutes");
    }
    // Only use defaults if no event data exists (new event creation)
    return type === "flexible" ? 60 : 30; // 60 min for flexible, 30 min for fluid
  };
  ...
  const [flexibleDuration, setFlexibleDuration] = useState(
    calculateActualDuration() // Use calculated duration instead of hardcoded
  );
  ...
  const [recurrenceEnabled, setRecurrenceEnabled] = useState(
    event?.recurrence?.enabled || false
  );
  ...
  const eventTypes = {
    fixed: {
      ...
      color: "#0081A7",
      bgColor: "#0081A720",
      description: "Exact date and time",
    },
    flexible: {
      name: "Flexible",
      color: "#00AFB9",
      bgColor: "#00AFB920",
      description: "Specific day, flexible time",
    },
    fluid: {
      name: "Fluid",
      color: "#F07167",
      bgColor: "#F0716720",
      description: "Anywhere this week",
    },
  };
  ...
  const generateWeekOptions = () => {
    ...
    for (let i = 0; i < 4; i++) {
      // Show 4 weeks: current + 3 future weeks
      const weekStart = today.clone().add(i, "weeks").startOf("week"); // Sunday
      const weekEnd = weekStart.clone().endOf("week"); // Saturday

      let label;
      if (i === 0) {
        label = `This week (${weekStart.format("MMM D")} - ${weekEnd.format(
          "MMM D"
        )})`;
      } else if (i === 1) {
        label = `Next week (${weekStart.format("MMM D")} - ${weekEnd.format(
          "MMM D"
        )})`;
      } else {
        label = `In ${i} weeks (${weekStart.format("MMM D")} - ${weekEnd.format(
          "MMM D"
        )})`;
      }
  ...
  const calculateEventTimes = () => {
    ...
    switch (type) {
      ...
      case "fluid": {
        ...
        if (fluidWeek === "current") {
          ...
          const weeksToAdd = parseInt(
            fluidWeek.replace("plus", "").replace("weeks", "")
          );
          targetWeekStart = now
            .clone()
        ...
        const fluidStart = targetWeekStart
          .clone()
          .add(1, "day")
          .add(9, "hours"); // Monday 9 AM
      ...
      default: {
        return {
          start: now.toDate(),
          end: now.clone().add(1, "hour").toDate(),
        };
      }
    }
  };
  ...
  const handleSubmit = (e) => {
    ...
    if (
      (type === "flexible" && flexibleDuration <= 0) ||
      (type === "fluid" && fluidDuration <= 0)
    ) {
      alert("Duration must be greater than 0");
      return;
    }

    const { start, end, duration, targetWeek, targetWeekStart } =
      calculateEventTimes();
